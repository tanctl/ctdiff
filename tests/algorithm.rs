...
fn test_input_size_limits() {
    ...
    // should succeed w small inputs
    let result = differ.diff(b"small", b"tiny");
    assert!(result.is_ok());
    
    // should fail w large inputs
...
fn test_edit_distance_limits() {
    ...
    // should succeed w small edit distance
...
fn test_padding_functionality() {
    ...
    let differ = ConstantTimeDiff::new(config);
    // test w same-length inputs first (easier case)
...
fn test_diff_result_validation() {
    ...
    // create invalid result w wrong metadata
...
fn test_error_handling() {
    // test apply w wrong input length
