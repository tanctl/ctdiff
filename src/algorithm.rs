...
//! Implements a modified myers algorithm that keeps execution time dependent on input sizes instead of content patterns
...
impl ConstantTimeDiff {
    ...
    /// returns edit script and metadata w execution time tied to input lengths
    pub fn diff(&self, a: &[u8], b: &[u8]) -> Result<DiffResult, DiffError> {
        // validate input sizes against configured limits
        ...
        // filter out padding operations for final result
    ...
    /// pad inputs to uniform size for steadier timing
    /// pads w a distinctive byte pattern that can be removed later
    ...
    /// runs the full matrix regardless of early results
    ...
    /// extract edit script using constant-time backtracking
    /// evaluates transitions without content-dependent branching
    ...
    /// scans entire operation list to keep timing uniform
        ...
        for op in operations {
            let (include_op, new_pos_a, new_pos_b) = match op {
                ...
                DiffOperation::Substitute(byte) => {
                    // include if within bounds and not substituting padding
                    // allow substitution while still within original bounds
...
#[cfg(test)]
    mod tests {
        ...
        #[test]
        fn test_simple_diff() {
            // use config with no padding for simpler testing
