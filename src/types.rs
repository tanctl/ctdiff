...
//! Defines core types used throughout the library while avoiding information leakage through type structure

use serde::{Deserialize, Serialize};

/// basic diff operation that can transform one sequence into another
/// operation encoding aims for uniform representation
...
pub enum DiffOperation {
    ...
    Delete,
    /// substitute byte at current position
    Substitute(u8),
}
...
pub struct DiffResult {
    ...
    /// original length of first input
    pub original_len_a: usize,
    /// original length of second input
...
impl DiffResult {
    ...
    /// verify that edit script is consistent w reported metadata
...
pub struct SecurityConfig {
    /// maximum input size to process (limits dos surface)
    pub max_input_size: usize,
    /// whether to pad inputs to uniform size
    pub pad_inputs: bool,
    ...
    /// maximum edit distance to compute
