...
//! Provides secure file comparison w timing attack resistance and familiar unix diff-style output formatting
...
enum SecurityLevel {
    ...
    Balanced,
    /// performance optimized w basic security
    Fast,
}
...
fn run_diff(cli: &Cli, file1: &PathBuf, file2: &PathBuf) -> Result<i32, Box<dyn std::error::Error>> {
    ...
    // perform diff w timing measurement
    ...
    if let (Some(file1_path), Some(file2_path)) = (file1, file2) {
        ...
        if let (Some(vuln_stats), Some(secure_stats)) = (
            ...
            if comparison.significant_difference {
                println!("files show significant timing differences - vulnerable to attack");
            } else {
