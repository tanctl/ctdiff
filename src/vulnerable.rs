...
//! Shows a traditional fast diff algorithm that intentionally leaks timing information for comparison

use crate::types::{DiffOperation, DiffResult, DiffError};

/// vulnerable diff implementation w timing side-channels
/// this implementation uses optimization techniques that create timing leaks:
/// - early termination stops as soon as differences are found
/// - short-circuit evaluation exits on first byte difference  
/// - content-dependent branching picks code paths based on input patterns
/// - optimized identical detection uses a fast path for identical inputs
/// these optimizations make the algorithm faster in many cases but leak information about input content through timing
...
impl VulnerableDiff {
    ...
    /// perform vulnerable diff w timing side-channels
    /// 
    ...
    pub fn diff(&self, a: &[u8], b: &[u8]) -> Result<DiffResult, DiffError> {
        // vulnerability: fast identical check with early return
        ...
        // vulnerability: content-dependent algorithm selection
    ...
    /// fast identical check w short-circuit evaluation
    ...
    fn fast_diff_algorithm(&self, a: &[u8], b: &[u8]) -> Result<DiffResult, DiffError> {
        ...
        // timing still reveals how much was processed
    ...
    fn slow_diff_algorithm(&self, a: &[u8], b: &[u8]) -> Result<DiffResult, DiffError> {
        ...
        // in reality this is a slightly different version with its own leaks
...
/// intentionally keeps timing side-channels for comparison
